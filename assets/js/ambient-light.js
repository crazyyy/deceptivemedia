...
jQuery(document).ready(function($) {
    ...
    if ($("#grid").length > 0) {
      ...
      setTimeout(function() {
        savvior.init('#grid', {
          ...
          "screen and (min-width: 400px) and (max-width: 1000px)": {
            columns: 3
          },
          ...
          if (comment_data.status.success === "true") {
            jQuery.ajax({
              ...
              success: function(result) {
                ...
                var AdminBarHeight = $('#wpadminbar').innerHeight();
                $("html, body").animate({
                  scrollTop: $('#comment-' + comment_data.data.comment_ID).offset().top - (HeaderHeight + AdminBarHeight + 10)
                }, AnimationSpeed);
  ...
  function gravatar_image() {
    $('#email').blur(function() {
      ...
      $(".gravitar-head-form").attr({
        src: "http://www.gravatar.com/avatar/" + get_gravatar('' + emailtrimmed + '', 94) + "?s=94&d=mm"
      });
    });
  }
  gravatar_image();
