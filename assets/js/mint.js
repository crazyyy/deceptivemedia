...
Mint.save = function() {
  ...
  var path = 'http://www.deceptivemedia.co.uk/mint/?record&key=333539336567756d4632434377446834354943757a52303531';
...
function Mint_SI_IO_isLocal(url) {
  return /^([^:]+):\/\/([a-z0-9]+[\._-])*(deceptivemedia\.co\.uk)/i.test(url);
};

function Mint_SI_IO_save() {
  var now = new Date();
  var then = now.getTime() + 300;
  var path = 'http://www.deceptivemedia.co.uk/mint/pepper/shauninman/outbound/click.php?' + now.getTime();
  path = path.replace(/^https?:/, window.location.protocol);
...
Mint.SI.UserAgent007 = {
  ...
  onsave: function() {
    if (this.flashVersion == this.versionHigh) {
      this.flashVersion = 0;
    };
